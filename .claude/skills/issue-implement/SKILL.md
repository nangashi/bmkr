...
allowed-tools: Bash, Read, Glob, Grep, Agent, Write, Edit, AskUserQuestion, Skill, EnterWorktree, ExitWorktree
...
Issue 番号とタイトルから適切なブランチ名を生成し、`EnterWorktree` で worktree を作成してセッションを切り替える。以降の全フェーズは worktree 内で実行される。中間ファイル（レポート等）の出力先は worktree 内の `.output/issue-implement/{issue_number}/` とする。
...
レビュー・修正が完了した実装を PR として提出する。`references/pr-creation.md` に従い、受け入れ条件チェック、コミット・プッシュ・PR 作成、worktree クリーンアップを行う。
