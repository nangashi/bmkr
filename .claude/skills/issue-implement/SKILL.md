...
Phase 6: 振り返り       → 空振り・手戻り検出、知見蓄積、改善提案、main復帰
```
...
Phase 5 完了後、または Phase 4 中断後に実行する。プロセスの過剰・不足を検出し、失敗・成功から得た知見を蓄積したうえで main ブランチに戻る。
...
失敗・指摘も成功パターンも1件もなかった場合は 6c をスキップし、6d に進む。
