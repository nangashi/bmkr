...
DB 生成型（sqlc の `db.Product` 等）をテンプレートやプレゼンテーション層に直接渡さない。レイヤー間はプレゼンテーション用の型を定義し、ハンドラで変換する。

### DB 依存はインターフェース経由で注入する

ハンドラが `*db.Queries` を直接フィールドに持つと、テストで DB をモックできない。必要なメソッドだけを持つインターフェースを定義し、ハンドラはそのインターフェースに依存させる。
