...
`git diff main` を取得し、以下のプロンプトを `codex exec --full-auto` に stdin で渡す:
...
レビュー結果は .output/issue-implement/{issue_number}/review-design-codex.md に書き出してください。
...
両レビューの出力を Read で読み込み、指摘を統合する。メインエージェントが I/F定義を修正し、Phase 3（テスト作成）に進む。
