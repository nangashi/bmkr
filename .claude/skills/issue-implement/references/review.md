...
I/F定義のみを対象に、変更容易性の観点でレビューする。実装前にレビューすることで修正コストを最小化する。

### サブエージェントに渡す情報

- Phase 1 で定義した型・インターフェース・シグネチャ・動作コメント（git diff で取得）
- 作業ディレクトリパス（既存コード読み取り用）
- 出力先: `.output/issue-implement/{issue_number}/review-design.md`
- 「Write で書き出すこと」

### 確認すべき観点

- **情報隠蔽**: DB 生成型（sqlc の `db.Product` 等）がプレゼンテーション層に漏れていないか。レイヤー間は専用の型で受け渡すべき
- **テスタビリティ**: DB 依存が具象型ではなくインターフェース経由で注入されているか
- **凝集度**: 1つのインターフェース/型が複数の関心を持っていないか
- **名前の正確性**: 型名・メソッド名・フィールド名が実態を正確に表しているか
- **既存パターン適合**: 周辺の既存コードと命名・構成が一致しているか（既存コードを Read して比較すること）
- **ADR 準拠**: `docs/adr/` の決定事項に反していないか

### 指摘があった場合

メインエージェントが I/F定義を修正し、Phase 3（テスト作成）に進む。
...
以下を並列で実行する。各サブエージェントには `git diff main` の差分と作業ディレクトリパスを渡し、結果を `.output/issue-implement/{issue_number}/` 配下にファイルで書き出すよう指示する。
...
LLM アンチパターン:
- **過剰抽象化**: 1実装しかないのにインターフェースを切っている（Phase 1 で定義したものは除く）、1箇所でしか使わないヘルパー関数を作っている
- **設定の過剰外部化**: 固定で良い値を環境変数や設定ファイルに切り出している
- **cargo cult エラーハンドリング**: 意味のない err wrap、catch して re-throw するだけ、冗長な nil チェック
