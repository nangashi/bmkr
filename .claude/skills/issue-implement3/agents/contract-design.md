...
- **Invariants**: 今回の変更で壊してはいけないこと（レスポンス形状、ルーティング、主要副作用、スコープ外制約を含める）
- **Generated Code Preconditions**: proto/sqlc/templ の前提
- **Acceptance Mapping**: 受け入れ条件と契約箇所の対応表

出力先: `.output/issue-implement3/{issue_number}/contract.md`

#### L2 からの昇格時

L2 の interface-design-decisions.md と failure_log が入力として提供される。白紙から設計せず、以下を活用する:

- L2 で定義済みの型・インターフェース・wip コメントはそのまま contract.md の Public Interface に取り込む
- failure_log から判明した設計上の問題点を Invariants や Error Contract に反映する
- L2 で作成済みのテストコードは破棄しない（Phase 3 で再評価する）
...
3. `.output/issue-implement3/{issue_number}/contract-decisions.md` に書き出す
