...
codex exec は外部プロセスとして実行されるため、サブエージェントのコンテキストを圧迫しない。codex 実行と対応合理性フィルタリングを1つのサブエージェントにまとめることで、メインコンテキストの節約と処理の一貫性を両立する。
...
### サブエージェントの処理フロー（詳細は agents/review-and-filter.md）:
1. **Phase 1**: codex exec でレビュー実行
2. **Phase 2**: 各指摘の対応合理性を判定し、価値の低い指摘を除外
3. **Phase 3**: 結果を構造化して `.output/codex-review/YYYYMMDD-{review_target}.md` に出力
