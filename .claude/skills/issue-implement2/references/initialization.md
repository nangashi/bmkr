...
- `git rev-parse --git-common-dir` と `git rev-parse --git-dir` を比較し、異なれば worktree 上で動作していると判定する。worktree 上でなければ以下を表示して終了する:
  ...
  ```
- `gh auth status` で GitHub CLI の利用可否を確認する
- `which codex` で codex CLI の利用可否を確認する。なければ以下を表示して終了する:
  ```
  codex CLI が見つかりません。
  このスキルは codex CLI（契約レビュー・テストレビュー・品質ゲートレビュー）を必須とします。
...
Issue本文およびコメントから実装の前提情報を読み込む。中間ファイルの出力先は `.output/issue-implement2/{issue_number}/` とする。

最低限、以下を `.output/issue-implement2/{issue_number}/issue-context.md` に記録する:
...
別ファイル `.output/issue-implement2/{issue_number}/scope.md` には以下を分離して記録する:
