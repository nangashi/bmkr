...
`references/contract-design.md` に従い、公開 I/F、エラー契約、不変条件、生成コード前提、受け入れ条件対応表を定義する。
...
`references/red-generation.md` に従い、契約からテストまたは検証項目を導出する。全テスト Green を一度に狙うのではなく、後段で failure cluster に分けやすい粒度で生成する。
