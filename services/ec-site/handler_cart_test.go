...
func TestAddItem(t *testing.T) {
	tests := []struct {
		...
		{
			name: "success with new cart creation via getOrCreateCart",
...
func TestRemoveItem(t *testing.T) {
	tests := []struct {
		...
		{
			name: "success: remove item (rows == 1, no GetCartItem pre-check)",
		...
		{
			name: "not found: item does not exist via rows == 0 (TOCTOU fix)",
...
func TestUpdateQuantity(t *testing.T) {
	tests := []struct {
		...
		{
			name: "not found: item does not exist via rows == 0",
...
// ---------------------------------------------------------------------------
// Tests: GetCart (getOrCreateCart helper paths)
// ---------------------------------------------------------------------------
...
func TestGetCart(t *testing.T) {
	tests := []struct {
		...
		{
			name: "success: existing cart found (getOrCreateCart returns on first GetCartByCustomerID)",
		...
		{
			name: "success: new cart created via getOrCreateCart (ErrNoRows → CreateCartIfNotExists → re-fetch)",
			req:  &ecv1.GetCartRequest{CustomerId: 100},
			querier: func() *mockCartQuerier {
				cart := testCart()
				items := testCartItems()
		...
		{
			name: "internal: CreateCartIfNotExists error in getOrCreateCart",
			req:  &ecv1.GetCartRequest{CustomerId: 100},
			querier: func() *mockCartQuerier {
				return &mockCartQuerier{
					getCartByCustomerIDFn: func(_ context.Context, _ int64) (db.Cart, error) {
		...
		{
			name: "internal: re-fetch after CreateCartIfNotExists fails",
			req:  &ecv1.GetCartRequest{CustomerId: 100},
			querier: func() *mockCartQuerier {
				callCount := 0
				return &mockCartQuerier{
					getCartByCustomerIDFn: func(_ context.Context, _ int64) (db.Cart, error) {
						...
						// Second call (re-fetch) also fails
						return db.Cart{}, errDB
					},
					createCartIfNotExistsFn: func(_ context.Context, _ int64) error {
						return nil
		...
		{
			name:     "invalid argument: customer_id negative",
			req:      &ecv1.GetCartRequest{CustomerId: -1},
			querier:  func() *mockCartQuerier { return &mockCartQuerier{} },
			wantCode: connect.CodeInvalidArgument,
			wantErr:  true,
		},
